...
<div id="searchresult-category-container">
	<c:if test="${symbol_dollar}{model.facetnav.count gt 0}">
		<div id="facets">
			<h4>Detail search criteria:</h4>
			<div class="form">
				<c:forEach var="facet" items="${symbol_dollar}{model.facetnav.folders}">
					<c:if test="${symbol_dollar}{facet.resultSet.count > 0}">
						<div class="fieldset">
							<c:choose>
								<c:when test="${symbol_dollar}{facet.leaf}">
									<h5 class="facetName"><c:out value="${symbol_dollar}{facet.name}" escapeXml="true" /></h5>
								</c:when>
								<c:otherwise>
									<h5 class="facetName"><c:out value="${symbol_dollar}{facet.name}" escapeXml="true" /></h5>
										...
										<c:forEach items="${symbol_dollar}{facet.folders}" var="item">
											<c:choose>
												<c:when test="${symbol_dollar}{item.leaf and item.count gt 0}">
													<hst:facetnavigationlink remove="${symbol_dollar}{item}" current="${symbol_dollar}{model.facetnav}" var="removeLink" />
													<div class="input selected">
														<span>
															<c:out value="${symbol_dollar}{model.labels[item.name]}" default="${symbol_dollar}{item.name}" escapeXml="true"/>
														</span> 
