...
public abstract class NodePickerController implements IDetachable {
    ...
    //TODO: 
    /** new constructor using Locale for changing configured opening location */
    public NodePickerController(IPluginContext context, NodePickerControllerSettings settings, Locale locale) {
        this.context = context;
        this.settings = settings;
        ...
        if(settings.hasBaseUUID()) {
            ...
            try {
            	Node baseNode = UserSession.get().getJcrSession().getNodeByIdentifier(baseUUID);
            	HippoNode nodeVariant = (HippoNode) getSourceNodeVariant(baseNode, locale);
                baseModel = new JcrNodeModel(nodeVariant.getCanonicalNode());
            } catch (RepositoryException e) {
                log.error("Could not create base model from UUID[" + baseUUID + "]", e);
            }
        }   
    ...
    private Node getSourceNodeVariant(final Node sourceNode, final Locale locale) {
        Node variant = sourceNode;
        try {
        	if(sourceNode.isNodeType("hippotranslation:translated")){
	        	HippoTranslatedNode hippoTranslatedNode = new HippoTranslatedNode(sourceNode);
	        	if(hippoTranslatedNode.hasTranslation(locale.toString())){
	        		variant = hippoTranslatedNode.getTranslation(locale.toString());
	        	}
        	}
        } catch (RepositoryException e) {
